...
Fsw_Simulation=Speed_Simulation*(pi/30)*(Poles/2)*(1/(2*pi));
...
Fsw_Actual=Speed_Actual*(pi/30)*(Poles/2)*(1/(2*pi));
...
subplot(3,2,6);
hold on;
plot(Torque_Simulation,Fsw_Simulation);
plot(Torque_Actual,Fsw_Actual,'r');
grid on;xlabel('Torque (N.m)');ylabel('Switching Frequency (Hz)');legend('Simulation','Actual');
